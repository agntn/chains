...
describe("chains MCP server", () => {
  ...
  it("resolves an alias to canonical metadata", async () => {
    ...
    expect(part?.text).not.toContain("pow:");
  });
  ...
  it("prints the magic bytes of a chain on Bitcoin's wire protocol", async () => {
    ...
    expect(part?.text).toContain("pow: sha256d");
  });
  ...
  it("resolves and validates Monero through MCP", async () => {
    const client = await connectTestClient();
    const address =
      "4AdUndXHHZ6cfufTMvppY6JwXNouMBzSkbLYfpAV5Usx3skxNgYeYTRj5UzqtReoS44qo9mtmXCqY45DJ852K5Jv2684Rge";
    const lookup = await client.callTool({ name: "chains_lookup", arguments: { chain: "XMR" } });
    expect(lookup.isError).not.toBe(true);
    ...
    expect(invalid.isError).not.toBe(true);
    expect(invalid.content).toEqual([
      { type: "text", text: `Invalid Monero (monero) address: "${address}0"` },
    ]);
  });

  it("resolves and validates Decred through MCP", async () => {
    const client = await connectTestClient();
    const address = "DsUZxxoHJSty8DCfwfartwTYbuhmVct7tJu";
    const lookup = await client.callTool({ name: "chains_lookup", arguments: { chain: "DCR" } });
    expect(lookup.isError).not.toBe(true);
  ...
  /** The lookup text is what a model reads before it converts an amount, so USDC and 18 have to be there. */
  it("looks Arc up through MCP with its USDC gas token", async () => {
    const client = await connectTestClient();
    const lookup = await client.callTool({
      name: "chains_lookup",
      arguments: { chain: "circle" },
    });
    expect(lookup.isError).not.toBe(true);
    const [part] = lookup.content as Array<{ text: string }>;
    expect(part?.text).toContain("Arc (arc)");
    expect(part?.text).toContain("symbol: USDC");
    expect(part?.text).toContain("decimals: 18");
    expect(part?.text).toContain("chainId: 0x13b2");
  ...
  /** Bitcoin SV shares Bitcoin's genesis and has no registered fork block, so Bitcoin's id stays off it. */
  it("looks Bitcoin SV up through MCP by its ticker", async () => {
    const client = await connectTestClient();
    const lookup = await client.callTool({
      name: "chains_lookup",
      arguments: { chain: "bsv" },
    ...
    expect(lookup.isError).not.toBe(true);
    const [part] = lookup.content as Array<{ text: string }>;
    expect(part?.text).toContain("Bitcoin SV (bitcoinsv)");
    expect(part?.text).toContain("bip44: 236");
    expect(part?.text).toContain("caip2: none");
    expect(part?.text).not.toContain("000000000019d6689c085ae165831e93");
  });

  it("looks Bitcoin Gold up through MCP by its ticker", async () => {
    const client = await connectTestClient();
    const lookup = await client.callTool({
      name: "chains_lookup",
      arguments: { chain: "btg" },
    ...
    expect(lookup.isError).not.toBe(true);
    const [part] = lookup.content as Array<{ text: string }>;
    expect(part?.text).toContain("Bitcoin Gold (bitcoingold)");
    expect(part?.text).toContain("bip44: 156");
    expect(part?.text).toContain("caip2: none");
  });

  it("looks Dash up through MCP with its own genesis", async () => {
    const client = await connectTestClient();
    const lookup = await client.callTool({
      name: "chains_lookup",
    ...
    expect(lookup.isError).not.toBe(true);
    const [part] = lookup.content as Array<{ text: string }>;
    expect(part?.text).toContain("Dash (dash)");
    expect(part?.text).toContain("bip44: 5");
    expect(part?.text).toContain("caip2: bip122:00000ffd590b1485b3caadc19b22e637");
  });

  it("looks Zcash up through MCP by its ticker", async () => {
    const client = await connectTestClient();
    const lookup = await client.callTool({
      name: "chains_lookup",
      arguments: { chain: "zec" },
    ...
    expect(lookup.isError).not.toBe(true);
    const [part] = lookup.content as Array<{ text: string }>;
    expect(part?.text).toContain("Zcash (zcash)");
    expect(part?.text).toContain("bip44: 133");
    expect(part?.text).toContain("caip2: bip122:00040fe8ec8471911baa1db1266ea15d");
  });
  ...
  it("narrows an unknown address down to its format family", async () => {
    ...
    expect(response.isError).not.toBe(true);
    const [part] = response.content as Array<{ text: string }>;
    expect(part?.text).toContain("matches 14 of 36 checked chains");
  ...
  it("does not report a base58 look-alike as a second chain", async () => {
    ...
    const [part] = response.content as Array<{ text: string }>;
    expect(part?.text).toContain("matches 1 of 36 checked chains");
  ...
  it("treats an address matching nothing as an answer, not a tool error", async () => {
    ...
    expect(response.isError).not.toBe(true);
    const [part] = response.content as Array<{ text: string }>;
    expect(part?.text).toContain('"nope" matches none of the 36 checked chains.');
  ...
  it("enumerates the registry so an agent never has to guess a chain name", async () => {
    ...
    expect(listing).toContain("36 chains registered.");
    expect(listing).toContain("dogecoin    DOGE   utxo    Dogecoin");
    expect(listing).toContain("bitcoincash BCH    utxo    Bitcoin Cash");
    expect(listing).toContain("bitcoinsv   BSV    utxo    Bitcoin SV");
    expect(listing).toContain("bitcoingold BTG    utxo    Bitcoin Gold");
    expect(listing).toContain("dash        DASH   utxo    Dash");
    expect(listing).toContain("zcash       ZEC    utxo    Zcash");
