...
/**
 * A chain whose display name shares no spelling with its key. Resolution used
 * to reach such a key only through a self-entry in the alias table, so
 * forgetting that entry made the canonical key throw.
