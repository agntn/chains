...
- `name`, `symbol`, `type` and `explorer` are required. `decimals`, `bip44`, `chainId`, `caip2`, `magic`, `pow` and `rpcDefault` are optional and mean unknown when left out, never a default.
