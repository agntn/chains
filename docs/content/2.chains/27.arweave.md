...
Transaction IDs are the same 32 bytes in the same encoding, so a string that fits could be either. This check tells you it's a well formed Arweave identifier, not that it's a wallet. `assertTxid` runs the same rule under its own error.
