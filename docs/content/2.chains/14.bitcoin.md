...
title: Bitcoin
icon: i-token-btc
description: Legacy, Bech32 and Bech32m addresses. SegWit is checked to the letter of BIP-173 and BIP-350, legacy down to its Base58Check checksum.
---
...
**Legacy** `1…` and `3…` addresses decode to 25 Base58Check bytes under version `0x00` or `0x05`, and the last four have to match a double SHA-256 of the other 21. One character off fails, the same as on the SegWit branch. A `1…` address also passes as [Bitcoin SV](/chains/bitcoinsv), which kept the version byte, so [identify](/guide/identify) names both chains for it.
